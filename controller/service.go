...
import (
	...
	"github.com/zhao890420/dc-gateway/dto"
...
func (service *ServiceController) ServiceList(c *gin.Context) {
	...
	for _, listItem := range list {
		...
		serviceAddr := "unknow"
		clusterIP := common.GetConfig().MustValue("cluster", "cluster_ip", "")
		//clusterIP := lib.GetStringConf("base.cluster.cluster_ip")
		//clusterPort := lib.GetStringConf("base.cluster.cluster_port")
		clusterPort := common.GetConfig().MustValue("cluster", "cluster_port", "")

		//clusterSSLPort := lib.GetStringConf("base.cluster.cluster_ssl_port")
		clusterSSLPort := common.GetConfig().MustValue("cluster", "cluster_ssl_port", "")
...
func (service *ServiceController) ServiceStat(c *gin.Context) {
	...
	for i := 0; i <= currentTime.Hour(); i++ {
		dateTime := time.Date(currentTime.Year(), currentTime.Month(), currentTime.Day(), i, 0, 0, 0, common.TimeLocation)
	...
	for i := 0; i <= 23; i++ {
		dateTime := time.Date(yesterTime.Year(), yesterTime.Month(), yesterTime.Day(), i, 0, 0, 0, common.TimeLocation)
		hourData, _ := counter.GetHourData(dateTime)
...
func (admin *ServiceController) ServiceAddTcp(c *gin.Context) {
	...
	if _, err := infoSearch.Find(c, common.DefaultDbPool, infoSearch); err == nil {
	...
	if _, err := tcpRuleSearch.Find(c, common.DefaultDbPool, tcpRuleSearch); err == nil {
	...
	if _, err := grpcRuleSearch.Find(c, common.DefaultDbPool, grpcRuleSearch); err == nil {
...
func (admin *ServiceController) ServiceUpdateTcp(c *gin.Context) {
	...
	detail, err := service.ServiceDetail(c, common.DefaultDbPool, service)
	if err != nil {
		middleware.ResponseError(c, 2002, err)
		return
	}
