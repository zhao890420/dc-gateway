...
func (admin *APPController) APPList(c *gin.Context) {
	...
	info := &dao.App{}
	list, total, err := info.APPList(c, common.DefaultDbPool, params)
	if err != nil {
...
func (admin *APPController) APPDetail(c *gin.Context) {
	...
	detail, err := search.Find(c, common.DefaultDbPool, search)
	if err != nil {
		middleware.ResponseError(c, 2002, err)
		return
	}
...
func (admin *APPController) APPDelete(c *gin.Context) {
	...
	info, err := search.Find(c, common.DefaultDbPool, search)
	if err != nil {
		middleware.ResponseError(c, 2002, err)
		return
	}
	info.IsDelete = 1
	if err := info.Save(c, common.DefaultDbPool); err != nil {
		middleware.ResponseError(c, 2003, err)
		return
	}
...
func (admin *APPController) AppAdd(c *gin.Context) {
	...
	if _, err := search.Find(c, common.DefaultDbPool, search); err == nil {
...
func (admin *APPController) AppStatistics(c *gin.Context) {
	...
	for i := 0; i <= time.Now().In(common.TimeLocation).Hour(); i++ {
